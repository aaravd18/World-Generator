...
public class TERenderer {
    ...
    public void drawTilesDark(TETile[][] world, List<Point> visiblePoints) {
        for (Point point : visiblePoints) {
            int tileX = point.x;
            int tileY = point.y;
            if (world[tileX][tileY] == null) {
                throw new IllegalArgumentException("Tile at position x=" + tileX + ", y=" + tileY
                        + " is null.");
            }
            world[tileX][tileY].draw(tileX + xOffset, tileY + yOffset);
        }
    }
